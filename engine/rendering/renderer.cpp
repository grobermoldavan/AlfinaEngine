...
namespace al::engine
{
    ...
    void Renderer::render_update() noexcept
    {
        initialize_renderer();
        {
            ...
            {
                FileHandle* vertGpassShaderSrc = file_sync_load(gFileSystem, construct_static_string(EngineConfig::DEFFERED_GEOMETRY_PASS_VERT_SHADER_PATH), FileLoadMode::READ);
                FileHandle* fragGpassShaderSrc = file_sync_load(gFileSystem, construct_static_string(EngineConfig::DEFFERED_GEOMETRY_PASS_FRAG_SHADER_PATH), FileLoadMode::READ);
                ...
                file_free_handle(gFileSystem, vertGpassShaderSrc);
                file_free_handle(gFileSystem, fragGpassShaderSrc);
            ...
            {
                FileHandle* vertDrawFramebufferToScreenShaderSrc = file_sync_load(gFileSystem, construct_static_string(EngineConfig::DRAW_FRAMEBUFFER_TO_SCREEN_VERT_SHADER_PATH), FileLoadMode::READ);
                FileHandle* fragDrawFramebufferToScreenShaderSrc = file_sync_load(gFileSystem, construct_static_string(EngineConfig::DRAW_FRAMEBUFFER_TO_SCREEN_FRAG_SHADER_PATH), FileLoadMode::READ);
                ...
                file_free_handle(gFileSystem, vertDrawFramebufferToScreenShaderSrc);
                file_free_handle(gFileSystem, fragDrawFramebufferToScreenShaderSrc);
            }
            {
